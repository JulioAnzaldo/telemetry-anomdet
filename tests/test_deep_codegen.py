...
from telemetry_anomdet.version import __version__  # noqa: E402
...
def test_every_generated_file_is_stamped_with_the_generator_version():
    """
    Provenance. A flight artifact must be traceable to what produced it, and
    the 0.x interface warning has to travel with the code rather than living
    only in the docs.
    """
    det, X = _fitted()
    files = generate_c(extract_kan_gdn(det), test_windows=X[:2])
    assert set(files) == {"kangdn.h", "kangdn.c", "kangdn_vectors.h"}

    for name, source in files.items():
        assert f"Generated by telemetry_anomdet {__version__}" in source, name
        assert "unstable until telemetry_anomdet 1.0.0" in source, name

    # The running firmware can report the version it was built from.
    assert f'#define KANGDN_VERSION "{__version__}"' in files["kangdn.h"]


def test_generation_is_reproducible():
    """
    No timestamp in the banner: regenerating from the same fitted detector
    reproduces the sources byte for byte, so a diff shows only real changes.
    """
    det, X = _fitted()
    extracted = extract_kan_gdn(det)
    first = generate_c(extracted, test_windows=X[:2])
    second = generate_c(extracted, test_windows=X[:2])
    assert first == second
