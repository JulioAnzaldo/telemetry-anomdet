...
def threshold_for_budget(errors: Sequence[float], budget: float = 0.01) -> dict:
    ...
    The budget is an upper bound, not a target. Fewer points are flagged when
    the data cannot resolve the fraction asked for, and ``flagged`` in the
    result reports what was actually reached.
    ...
    # method="higher" rather than numpy's default linear interpolation. Linear
    # puts the cutoff between two samples, so more than the budget can sit above
    # it even when the budget is resolvable: 50 points at budget=0.05 flags 3
    # (0.06) where 2 (0.04) would fit. Snapping up to a real sample is what
    # makes the budget a bound rather than an approximation, at a cost of one
    # flagged point on large inputs.
    threshold = float(np.quantile(errors, 1.0 - budget, method="higher"))
